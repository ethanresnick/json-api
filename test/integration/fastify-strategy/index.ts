...
import FastifyStrategy from "../../../src/http-strategies/Fastify";

describe("Fastify Strategy", () => {
  let Agent;
  before(() => {
  ...
  describe("configuration", () => {
    ...
    it("does not require a documentation controller", () => {
      expect(() => new FastifyStrategy(api)).to.not.throw();
    });

    it("throws if you attempt to get a docs request handler if no docs controller was provided", () => {
      const fastify = new FastifyStrategy(api);
      expect(() => fastify.docsRequest).to.throw(/^Cannot get docs request handler/);
    });
  });
