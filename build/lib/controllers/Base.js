...
(function(){
  var Q, mongoose, bodyParser, templating, Document, Collection, ErrorResource, utils, config, BaseController, slice$ = [].slice;
  Q = require('q');
  ...
  BaseController = (function(){
    ...
    function BaseController(registry){
      ...
      this.jsonBodyParser = bodyParser.json({
        type: ['json', config.specMediaType]
      });
    }
    prototype.GET = function(req, res, next){
      var type, adapter, model, sorts, filters, fields, res$, k, ref$, includes, this$ = this;
      if (!req.accepts([config.specMediaType, 'application/json'])) {
        res.status(406);
        return res.send();
      }
      ...
      if (req.query.fields != null) {
        res$ = {};
        for (k in ref$ = req.query.fields) {
          fields = ref$[k];
          res$[k] = fields.split(',').map(decodeURIComponent);
        }
        fields = res$;
      }
      ...
      return Q.Promise(function(resolve, reject){
        ...
        if (req.params.id != null) {
          ...
          idOrIdsPromise = typeof mapper === "function"
            ? Q.all(idOrIdsRaw.map(partialize$.apply(this, [mapper, [void 8, model, req], [0]])))
            : Q(idOrIdsRaw);
