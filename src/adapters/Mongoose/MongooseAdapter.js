...
export default class MongooseAdapter {
  ...
  static getStandardizedSchema(model) {
    ...
    // Array[ModelNameId].
    const getStandardTypeString = (path, schemaType) => {
      if(path === "_id") {
        return "Id";
      }
    ...
    model.schema.eachPath((name, type) => {
      if(arrayContains([schemaOptions.versionKey, schemaOptions.discriminatorKey], name)) {
        return;
      }

      const standardTypeString = getStandardTypeString(name, type);
      name = (name === "_id") ? "id" : name;
      const likelyAutoGenerated = (
        standardTypeString === "Date" &&
